renderers.blocks.Q_Clock = function (b, a, c) {
    this.rotation = this.targetRotation = 0;
    this.randomize = !0;
    renderers.Q_Base.call(this, b, a, c);
}
m["renderers.blocks.Q_Clock"] = renderers.blocks.Q_Clock
renderers.blocks.Q_Clock.__name__ = "renderers.blocks.Q_Clock"
renderers.blocks.Q_Clock.__super__ = renderers.Q_Base
renderers.blocks.Q_Clock.prototype = z(renderers.Q_Base.prototype, {
    update: function () {
        ...
        (this.randomize = 1 != Main.Instance.game.world.sceneNum) ? (0 == (G.toFloat(Main.Instance.tick) % G.toFloat(8) | 0) && (this.targetRotation = Math.random() * Math.PI * 2), this.targetRotation < this.rotation - Math.PI ? this.rotation -= 2 * Math.PI : this.targetRotation > this.rotation + Math.PI && (this.rotation += 2 * Math.PI), this.rotation += (this.targetRotation - this.rotation) / 10) : this.rotation = -Main.Instance.game.world.tim / 100 * Math.PI * 2 + Math.PI / 2;
        this.entity.updateQuad(this.quadPositions[0], null, new lemongine.Point(this.textureClip.x, this.textureClip.y), new lemongine.Point(this.textureClip.width, this.textureClip.height), null, lemongine.Geom.quadMatrixHelper(new lemongine.Rectangle(0, 0, 1, 1), new lemongine.Matrix4().translate(-.5, -.5).rotate2D(this.rotation).scale2D(1.08).translate(.5, .5).scale2D(1).translate(this.destination.x, this.destination.y)));
    ...
    __class__: renderers.blocks.Q_Clock
})